...
export default function ProjectCard({ title, category, number, index, totalCards, images }: ProjectCardProps) {
  ...
  return (
    <div ref={containerRef} className="h-[85vh] w-full flex items-center justify-center sticky top-24 mb-24">
      <div 
        className="w-full h-full border-2 border-[#FF5949] rounded-[60px] p-8 bg-[#0C0C0C] flex flex-col gap-6"
        style={{ 
      ...
      >
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center gap-4 text-[#FF5949] text-xl font-medium uppercase tracking-widest">
            <span>{number}</span>
            <span>—</span>
            <span>{category}</span>
          </div>
          <h3 className="text-4xl md:text-5xl font-black text-[#FF5949] uppercase tracking-tight">{title}</h3>
          <LiveProjectButton />
        </div>
